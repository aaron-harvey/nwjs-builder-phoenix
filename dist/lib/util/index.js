...
function findRuntimeRoot(platform, runtimeDir) {
    return new Promise(function (resolve, reject) {
        ...
        var options = { cwd: runtimeDir, nodir: false };
...
function tmpFile(options) {
    if (options === void 0) { options = {}; }
    return new Promise(function (resolve, reject) {
        tmp.file(Object.assign({}, {}, options), function (err, path, fd, cleanup) { return err ? reject(err) : resolve({
...
function spawnAsync(executable, args, options) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        if (child.stderr) {
            child.stderr.on('data', function (chunk) { return debug('in spawnAsync', 'stderr', chunk.toString(), executable, args); });
        }
...
function fileExistsAsync(path) {
    return new Promise(function (resolve, reject) {
        fs_1.exists(path, resolve);
    });
}
exports.fileExistsAsync = fileExistsAsync;
function getPathFromObj(path, obj, fb) {
 ...
 * @param fallback
 */
function parseTmpl(template, obj) {
    return template.replace(/\$\{.+?}/g, function (match) {
        var path = match.substr(2, match.length - 3).trim();
        return getPathFromObj(path, obj);
    });
}
exports.parseTmpl = parseTmpl;
/**
 * Sort paths from depest to shallowest
 * @param {string[]} pathList
 * @returns {string[]} paths sorted from deepest to shallowest
 */
function sortByDepth(pathList) {
    var depths = pathList.map(function (pathElement) {
        return [pathElement.split(path_1.sep).length, pathElement];
    });
    var sortedDepths = depths.sort(function (_a, _b) {
        var countA = _a[0], pathElementA = _a[1];
        var countB = _b[0], pathElementB = _b[1];
        return countA > countB ? -1 : countA < countB ? 1 : 0;
    });
    return sortedDepths.map(function (elem) { return elem[1]; });
}
